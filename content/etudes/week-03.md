...
Here are some short studies for the vocabulary in Strudel's [first sounds](https://strudel.cc/workshop/first-sounds/)
workshop. Each one reduces most of the choices so you
...
For these etudes you'll only be uses the features listed below:
...
1. Write four variations. Each has to sound clearly different from the other
   three when played back to back. Comment out the other three so you can play one at a time.
...
1. First, prove to yourself that a rest is not the same as nothing. Play
   ...
   everything speeds up. (`~` is the same as `-`)
...
**Listen for:** the rests are doing the composing here. 
...
**Listen for:** when all three voices are busy the pattern sounds cluttered, and
when all three are sparse it stops holding together. 
...
**Listen for:** layers with different repeat lengths (3 and 2) only realign every 6 cycles; this is a cheap way to make a short pattern feel longer.
...
The response keeps the hats, the bank and the four-slot frame, then moves the
weight to the front of the cycle and answers the snare with a rest.
